...
bool inputHandler::isKeyPressed(int ka) const
{
	if (_kb._c[ka] != 0)
		return isKeyPressed(_kb._c[ka]);
	else
		return isKeyPressed(_kb._vk[ka]);
}
...
buttonPtr inputHandler::addButton(string name, colorType color, int x, int y, buttonAction action, int data)
{
	auto btn = buttonPtr(new button(name, color, x, y, action, data));
	return btn;
}

bool inputHandler::isMouseOverButton(buttonPtr btn) const
{
	auto mpt = getMouseCoord();
	return mpt.second == btn->_y && mpt.first >= btn->_x && mpt.first <= btn->_x + btn->_name.size();
}

buttonPtr inputHandler::getButtonUnderMouse() const
{
