...
{
	//	'dumb' monsters don't check for safety; they think every square is safe
	if (!cr->isPlayer() && static_pointer_cast<monster>(cr)->hasFlag("dumb"))
		return true;
	...
	//	deep water is bad unless we can swim
	if (tl == TILE_WATER_DEEP)
		return cr->immuneToDrowning();


	//	surface test
	auto sf = gdata->_map->getSurface(pt);
...
//	If there's a monster under the cursor, view a detailed summary of them
void tryExamineMonster(gamedataPtr gdata)
{
	auto t = gdata->_map->getCreature(gdata->_cursorPt);
	if (t != nullptr && canPlayerSeeCreature(gdata, t))
	{
		gdata->_state = STATE_EXAMINE_MONSTER;
		gdata->_target = t;
	}
}
...
{
	return gdata->_state == STATE_LOOK || gdata->_state == STATE_AIM_RANGED || gdata->_state == STATE_AIM_THROWN;
}
...
{
	return gdata->_map->isInFov(mon->_pos);
}
